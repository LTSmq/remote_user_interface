...
class MonitorDisplay(Frame):
    ...
    def update_info(self, info: dict) -> None:
        if "current_position" in info.keys():
            try:
                position = float(info["current_position"])
            except ValueError:
                return
        else:
            return
...
class HistoryPanel(Frame):
    ...
    def update_info(self, info: dict) -> None:
        for key, value in info.items():
            self.listbox.insert(END,f"{now()} - {str(key)} -> {str(value)}")
...
class BridgeControllerGUI(Tk):
    def __init__(self, interface: RemoteInterfaceHeader, title: str = "Bridge Controller GUI",
        ...
        self._build()
        self._update_info({"current_position": 0})
        self._refresh()
    ...
    def set_bridge_position(self, position: float) -> None:
        if self.override_panel.check_panel_status.value.upper() == "AUTOMATIC":
            return
        self.interface.execute("set_bridge_position", position=round(position))
    ...
    def enable_overrides(self):
        self.interface.execute("set_overrides", to=True)
    ...
    def disable_overrides(self):
        self.interface.execute("set_overrides", to=False)
    ...
    def sync_bridge_state(self):
        ...
        if position_info["response"] == "DATA":
